...
class XrayAPI:
    ...
    def login(self) -> bool:
        ...
        try:
            ...
            # Логируем тело ответа на неуспешный логин целиком — это
            # единственный способ понять реальную причину 403/401 (бан
            # IP за неудачные попытки, требование 2FA, неверный пароль
            # и т.п.), не гадая вслепую.
            logger.error(f"❌ Ошибка авторизации. Тело ответа панели: {response.text[:500]!r}")
