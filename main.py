...
PANEL_USERNAME = os.environ.get("PANEL_USERNAME", "fHRTAk9lFz")
PANEL_PASSWORD = os.environ.get("PANEL_PASSWORD", "pM0xjYSy4N")
API_TOKEN = os.environ.get("API_TOKEN", "tlZpRhpGQA54Uyta9p9chp42oymKG8NjoauzprvEqHSHqrye")
...
# ======================= РАБОТА С ПАНЕЛЬЮ 3X-UI (ИСПРАВЛЕННАЯ) =======================
async def create_vpn_user(telegram_id: int, days: int) -> Optional[str]:
    """Создаёт VPN-ключ через API 3x-ui с правильным форматом"""
    username = f"user_{telegram_id}_{int(datetime.now().timestamp())}"
    expiry_date = datetime.now() + timedelta(days=days)
    expiry_timestamp = int(expiry_date.timestamp() * 1000)  # В миллисекундах!
    
    # Правильный формат данных для 3x-ui API
    client_data = {
        "email": username,
        "limitIp": 5,
        "totalGB": 0,
